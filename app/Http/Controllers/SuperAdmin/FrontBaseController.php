...
class FrontBaseController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
            ...
            $this->localeLanguage = LanguageSetting::where('language_code', $this->locale)->first() ?: $this->enLocaleLanguage;

            $this->footerSettings = FooterMenu::whereNotNull('slug')->where('private', 0)->where('language_setting_id', $this->localeLanguage->id)->get();
            $this->footerSettings = $this->footerSettings->count() > 0 ? $this->footerSettings : FooterMenu::whereNotNull('slug')->where('private', 0)->where('language_setting_id', $this->enLocaleLanguage->id)->get();
